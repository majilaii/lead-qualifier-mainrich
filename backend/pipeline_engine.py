...
async def process_companies(
    ...
    *,
    ...
    """
    Process a list of companies through crawl → qualify → extract contacts.
    Emits events to the PipelineRun for real-time streaming.

    This is the shared core that all entry points (chat, manual config,
    bulk import, API, scheduled) use.

    Args:
        companies: List of company dicts [{url, domain, title, score?}, ...]
        search_ctx: Search context dict (industry, criteria, etc.)
        use_vision: Whether to use vision (screenshots) for qualification
        ...
        async with CrawlerPool() as pool:
            for i, company in enumerate(companies):
                try:
                    ...
                    # Sniff contact pages if main crawl succeeded
                    ...
                    if not crawl_result.success:
                        fail_country = country_from_domain_fn(company.get("domain", "")) if country_from_domain_fn else None
                        fail_lat, fail_lng = None, None
                        if fail_country and geocode_fn:
                            geo = await geocode_fn(fail_country)
                            if geo:
                                _, fail_lat, fail_lng = geo
                        fail_lat, fail_lng = spread(fail_lat, fail_lng)

                        error_msg = sanitize_error_fn(crawl_result.error_message) if sanitize_error_fn else "Website unreachable"
                        result_data = {
                            "title": company["title"],
                            "domain": company["domain"],
                            "url": company["url"],
                            "score": 5,
                            "tier": "review",
                            "reasoning": f"Website could not be crawled — {error_msg}. This company may still be relevant; visit the site manually to verify.",
                            "hardware_type": None,
                            "key_signals": [],
                    ...
                    if search_id and save_lead_fn:
                        try:
                            await save_lead_fn(search_id, result_data, user_id=user_id, contacts=extracted_contacts)
                        except Exception as e:
